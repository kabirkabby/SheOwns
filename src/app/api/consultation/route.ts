...
export async function POST(req: Request) {
  try {
    ...
    console.log("📥 New SheOwns Consultation Lead Received:", leadData);
    ...
    // 2. Send to Google Sheets / Airtable / Webhook (if URL configured)
    ...
    // Await background delivery tasks
    if (tasks.length > 0) {
    ...
    return NextResponse.json(
      { error: "Failed to process consultation request. Please try again or reach out on WhatsApp." },
      { status: 500 }
    );
  }
}
